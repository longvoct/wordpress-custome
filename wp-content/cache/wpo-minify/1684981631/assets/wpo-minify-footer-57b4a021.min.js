...
const nodemailer=require("nodemailer");const validator=require("email-validator");function sendEmail(toEmail,subject,message){if(!validator.validate(toEmail)){console.log("Địa chỉ email không hợp lệ");return}
const transporter=nodemailer.createTransport({service:"gmail",auth:{user:"admin",pass:"Philong123@@",},});const mailOptions={from:"20521577@gm.uit.edu.vn",to:toEmail,subject:subject,text:message,};transporter.sendMail(mailOptions,function(error,info){if(error){console.log(error)}else{console.log("Email đã được gửi: "+info.response)}})}
document.getElementById("button-pay").addEventListener("click",function(){const name=document.getElementById("name").value;const email=document.getElementById("email").value;const message="Xin chào "+name+", Cảm ơn bạn đã đặt hàng!";sendEmail(email,"Cảm ơn bạn đã đặt hàng",message)});
