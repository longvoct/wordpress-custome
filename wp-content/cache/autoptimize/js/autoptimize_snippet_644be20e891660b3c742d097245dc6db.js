...
var thwvsf_lazy_load = (function () {
    ...
    function initialize_thwvsf_lazy_load() {
      function showImagesOnView(e) {
        ...
        try {
          for (
            var _step, _iterator = images[Symbol.iterator]();
      ...
      if ("yes" === thwvsf_public_var.lazy_load) {
        var images = document.querySelectorAll("img.swatch-image[data-src]");
        showImagesOnView(),
          window.addEventListener("scroll", showImagesOnView, !1);
      }
    }
    return { initialize_thwvsf_lazy_load: initialize_thwvsf_lazy_load };
  })(),
  thwvsf_public_base = (function ($, window, document) {
    "use strict";
    ...
    function set_field_value_by_elm(elm, type, value) {
      switch (type) {
        ...
        case "multiselect":
          elm.prop("multiple")
            ? void 0 !== value && elm.val(value.split(",")).change()
            : elm.val([value]);
          break;
        default:
          elm.val(value);
  ...
  thwvsf_public = (function ($) {
    ...
    function initialize_thwvsf() {
        ...
        $attributeFields.each(function (index, el) {
          ...
          if (current_attr_select.hasClass("wc-bundle"))
            var current_attr_name =
              "attribute_" + current_attr_select.attr("id");
          form.$form
            .find(
              '.thwvsf-checkbox[data-attribute_name= "' +
                current_attr_name +
                '"]'
            )
            .addClass("deactive"),
      ...
      function disable_out_of_stock_variation(form) {
        ...
        if (1 == attributeFields.length)
          for (
            var variations = form.variationData, i = 0;
            i < variations.length;
            i++
          ) {
            ...
            $(attributeFields).hasClass("wc-bundle") &&
              (attr_item_name_class =
                "attribute_" + current_attr_select.attr("id")),
              (attr_item_name_class = attr_item_name_class.replace(
                /[^a-z0-9_-]/gi,
                ""
              ));
            var attribute_value = variation_attributes[attr_item_name],
              attribute_value_class = attribute_value.replace(
                /[^a-z0-9_-]/gi,
                ""
              ),
            ...
            attribute_value_class
              ? ((attr_option_class = "." + attr_item_name_class),
                (attr_option_class =
                  attr_option_class + "." + attribute_value_class))
              : (attr_option_class =
                  "." +
                  attr_item_name_class +
                  '[data-value="' +
      ...
      function disable_out_of_stock_variation_multiple(form, attributeFields) {
        ...
        attributeFields.each(function (index, element) {
          ...
          (0 == count || count < total_attributes - 1) &&
            form.$form
              .find(".thwvsf_fields .thwvsf-checkbox")
        ...
        if (count == total_attributes - 1)
          for (var i = 0; i < variations.length; i++) {
            ...
            $.each(
              ...
              function (attr_item_name, attribute_value) {
                if (
                  variation_attributes[attr_item_name] ==
                    selected_terms[attr_item_name] &&
                  ++q == total_attributes - 1
                ) {
                  ...
                  for (var current_attr_name in current_attributes)
                    if (
                      -1 ==
                      jQuery.inArray(current_attr_name, selected_term_names)
                    ) {
                      var current_attr_val =
                          variation_attributes[current_attr_name],
                        attr_item_name_class = current_attr_name.replace(
                          /[^a-z0-9_-]/gi,
                          ""
                        ),
                        current_attr_select = $(
                          "select.cls_" + attr_item_name_class
                        );
                      current_attr_select.hasClass("wc-bundle") &&
                        (attr_item_name_class =
                          "attribute_" + current_attr_select.attr("id")),
                        (attr_item_name_class = attr_item_name_class.replace(
                          /[^a-z0-9_-]/gi,
                      ...
                      var attribute_value_class = current_attr_val.replace(
                          /[^a-z0-9_-]/gi,
                          ""
                        ),
                        attr_option_class = "";
          ...
          for (var i = 0; i < variations.length; i++) {
            var variation = variations[i],
              variation_attributes = variation.attributes,
              is_in_stock = variation.is_in_stock;
            if (!is_in_stock && "default" != out_of_stock) {
              var q = 0,
                current_variation = variation,
                current_attributes = current_variation.attributes;
              $.each(
                current_attributes,
                function (os_attr_name, os_attr_value) {
                  selected_terms[os_attr_name] == os_attr_value &&
                    ++q == total_attributes - 1 &&
                    $.each(
                      current_attributes,
                      function (de_attr_name, de_attr_value) {
                        if (selected_terms[de_attr_name] != de_attr_value) {
                          var current_os_attr_val =
                              current_attributes[de_attr_name],
                            attr_item_name_class = de_attr_name.replace(
                              /[^a-z0-9_-]/gi,
                              ""
                            ),
                            attribute_value_class = current_os_attr_val.replace(
                              /[^a-z0-9_-]/gi,
                              ""
                            ),
                            current_attr_select = $(
      ...
      function show_selected_variation_name_beside_label(
        ...
        elm
          ...
          .find(".label")
          .find("label")
          .find(".variation_name_label")
          .remove();
        var default_label = elm.closest("tr").find(".label").find("label"),
          variation_name_label_html =
            '<label class="variation_name_label" > ' +
            thwvsf_public_var.change_separator +
            variation_name +
            " </label>";
        (variation_name_label_html = name_value
          ? variation_name_label_html
          : ""),
      ...
      function format_swatch_color(option) {
        ...
        return $(
          '<span class = "thwvsf-drop-span thwvsf-drop-color" style="background-color:' +
            color +
            ';"> </span><span class = "thwvsf-drop-label" >' +
            option.text +
            "</span>"
        );
      }
      function format_swatch_image(option) {
        ...
        return $(
          '<span class = "thwvsf-drop-span thwvsf-drop-img"><img src="' +
            image +
            '" /> </span><span class = "thwvsf-drop-label" >' +
            option.text +
            "</span>"
      ...
      var clear_on_reselect =
          (thwvsf_public_var.enable_stock_alert,
          thwvsf_public_var.min_value_stock,
          thwvsf_public_var.clear_on_reselect),
        out_of_stock = thwvsf_public_var.out_of_stock,
        show_selected_variation_name =
          thwvsf_public_var.show_selected_variation_name,
        ...
        swatches_form = function ($form) {
          ...
          (self.$form = $form),
            ...
            $form.on(
              "click.thwvsf_variation_form",
              ".thwvsf-checkbox",
              { swatches_form: this },
              this.onselect
            ),
            $form.on(
              "change.thwvsf_variation_form",
              'input[type="radio"].thwvsf-rad',
              { swatches_form: this },
              this.onselectradio
            ),
            $form.on(
              "check_variations.thwvsf_variation_form",
              { swatches_form: this },
            ...
            $form.on(
              "change.thwvs_variation_form",
              ".variations .thwvs-select",
              { swatches_form: this },
              this.onchangeselect
            ),
            thwvsf_public_var.selectWoo_enable &&
              self.enableSwatchDropDown(self.$attributeFields, $form);
          var selc2_el = $(".thwvsf-wrapper-ul").prev();
          selc2_el.hasClass("select2") && selc2_el.css("display", "none");
          var selc2_rad_el = $(".thwvsf-rad-li").prev();
          selc2_rad_el.hasClass("select2") &&
      ...
      (swatches_form.prototype.onReset = function (event) {
        ...
        form.$form
          ...
          form.$form
            .find(".thwvsf_fields .thwvsf-checkbox")
            .removeClass("out_of_stock"),
          form.$form.find(".thwvsf-rad").prop("checked", !1),
          form.$form.find(".thwvsf-rad").attr("checked", !1),
          form.$form
            .find(".thwvsf-rad-li > label")
            .removeClass("thwvsf-selected");
        var $element = $(this);
        $element
          .parents(".variations_form")
          .siblings(".thwvsf_add_to_cart_button");
        ...
        (swatches_form.prototype.onselect = function (event) {
          ...
          if (
            ((opt_val =
              "string" == typeof opt_val
                ? opt_val.replace(/'/g, "\\'").replace(/"/g, '\\"')
                : opt_val),
            !$select.find('option[value="' + opt_val + '"]').length)
          )
            return (
              $element
                .siblings(".thwvsf-checkbox")
                .removeClass("thwvsf-selected"),
              $select.val("").change(),
        ...
        (swatches_form.prototype.onselectradio = function (event) {
          var $element = (event.data.swatches_form, $(this)),
            $select = $element.closest(".thwvsf_fields").find("select"),
            attribute_name =
              $select.data("attribute_name") || $select.attr("name"),
            value = $element.data("value");
        ...
        (swatches_form.prototype.onchangeselect = function (
          event,
          chosenAttributes
        ) {
          var $element = $(this);
          if ("yes" === show_selected_variation_name) {
            show_selected_variation_name_beside_label(
              $element,
              $element.children(":selected").text(),
              $element.val()
            );
          }
        }),
        ($.fn.wc_set_variation_attr = function (attr, value) {
          void 0 === this.attr("data-o_" + attr) &&
            this.attr("data-o_" + attr, this.attr(attr) ? this.attr(attr) : ""),
            !1 === value ? this.removeAttr(attr) : this.attr(attr, value);
        }),
        (swatches_form.prototype.onFindVariation = function (event) {
          var form = event.data.swatches_form,
            $attributeFields = form.$attributeFields;
          active_and_deactive_variation(form),
            disable_out_of_stock_variation(form),
            "yes" === show_selected_variation_name &&
              $attributeFields.each(function (index, element) {
                var current_attr_select = $(this);
                current_attr_select.data("attribute_name") ||
                  current_attr_select.attr("name");
                "" != current_attr_select.val() &&
                  show_selected_variation_name_beside_label(
                    current_attr_select,
                    current_attr_select.children(":selected").text(),
                    current_attr_select.val()
                  );
              });
        }),
        (swatches_form.prototype.enableSwatchDropDown = function (
          $attributeFields,
          $form
        ) {
          $attributeFields.each(function (index, el) {
            ...
            if (attr_select.hasClass("thwvsf-swatch-dropdown")) {
              "swatch_dropdown_color" === attr_select.data("swatchtype")
                ? attr_select
                    .selectWoo({
                      allowClear: !0,
                      ...
                      selectionAdapter: $.fn.selectWoo.amd.require(
    ...
    return (
      initialize_thwvsf(),
      "flatsome" == thwvsf_public_var.is_quick_view
        ? $(document).on("click", ".quick-view", function () {
            execute ||
              $(document).on("mfpOpen", function () {
                initialize_thwvsf(), (execute = !0);
              });
          })
        : "yith" == thwvsf_public_var.is_quick_view &&
          $(document).on("qv_loader_stop", function () {
            initialize_thwvsf();
          }),
      "wpc_smart" == thwvsf_public_var.is_quick_view &&
        $(document).ajaxComplete(function (event, request, options) {
          request &&
            4 === request.readyState &&
            200 === request.status &&
            $(event.target).find("#woosq-popup").length > 0 &&
            initialize_thwvsf();
        }),
      "porto" === thwvsf_public_var.is_quick_view &&
        $(document).on("porto_init_countdown", function () {
          initialize_thwvsf();
        }),
      "woodmart" === thwvsf_public_var.is_quick_view &&
        $(document).on("wdQuickShopSuccess", function () {
          initialize_thwvsf();
        }),
      $(document).on("click", ".owp-quick-view", function (e) {
        var check = function () {
          $("#owp-qv-wrap").hasClass("is-visible")
            ? init_thwvsf()
            : setTimeout(check, 1e3);
        };
        check();
      }),
      "pi_dcw" === thwvsf_public_var.is_quick_view &&
        $(document).on("click", ".pisol_quick_view_button", function () {
          $(document).ajaxComplete(function (event, request, options) {
            request &&
              4 === request.readyState &&
              200 === request.status &&
              $(event.target).find(".pisol-quick-view-box").length > 0 &&
              initialize_thwvsf();
          });
        }),
